...
seat_grid.py
------------
A 2-D array representation of the seats inside a single zone.

Each cell stores either None (free) or a user ID (occupied).
Indexed seat access is O(1).
"""
...
class SeatGrid:
    """Row-major 2-D array of seats, with O(1) access."""
    ...
    def free_count(self) -> int:
        return sum(1 for row in self._cells for c in row if c is None)
    ...
    def seats_for_user(self, user_id: str) -> List[str]:
        out = []
        for r, c, holder in self.all_seats():
            if holder == user_id:
                out.append(self.seat_code(r, c))
        return out
